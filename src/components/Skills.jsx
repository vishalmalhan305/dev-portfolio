      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <h2 className="text-4xl font-bold mb-4 flex items-center justify-center gap-3">
            <Zap className="w-10 h-10 text-yotei-gold" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {skills.map((skill, index) => (
            <div 
              key={index} 
              className="skill-card bg-gradient-to-br from-yotei-black to-yotei-dark-gray p-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 text-center border border-yotei-gold/30 hover:border-yotei-gold/80 group cursor-pointer"
            >
              <h3 className="text-sm md:text-base font-bold text-yotei-gold group-hover:text-white transition-colors duration-300">
                {skill}
              </h3>
            </div>
          ))}
